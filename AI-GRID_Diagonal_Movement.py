...
class XYStepper:
    def __init__(self, root):
        self.root = root
        self.root.title("XY Stepper Controller")
        self.output = None
    ...
    def init_ui(self):
        ...
        tk.Button(self.root, text="Start 2mm Movement", command=self.start_2mm_movement).grid(row=6, column=0)
    ...
    def perform_step(self, step):
        if self.stop_requested:
            ...
            return
        if step < 2:
            print("Moving 1mm step {}/2".format(step + 1))
            self.move_1mm()
            self.root.after(30000, lambda: self.wait_until_ready(step))  # Schedule next step after 30 seconds
        else:
            print("Completed 2mm movement.")

    def wait_until_ready(self, step):
        try:
            while True:
                received = self.a.readline().strip()
                print("Received: {}".format(received))  # Debug statement
                if received == 'Ready':
                    print("Pausing for 30 seconds after step {}/2".format(step + 1))
                    self.root.after(30000, lambda: self.perform_step(step + 1))
                    break
        except serial.SerialException as e:
    ...
    def move_1mm(self):
        # Function to move 1mm in each direction
        x_val = self.cx + (self.steps_per_mm // 2)
        y_val = self.cy + (self.steps_per_mm // 2)
        coordinate = 'Cx{}y{}'.format(int(x_val), int(y_val))
