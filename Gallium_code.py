...
class XYStepper:
    def __init__(self, root):
        ...
        self.output = None
        # Calculate steps per mm for stepper motor
        self.steps_per_mm = int(6 / 0.5 * 1600)
    ...
    def connect_serial(self):
        self.a = serial.Serial('COM4', baudrate=9600,
                               timeout=20)  # Open serial port
...
###########################################################################################################################
    ...
    def init_ui(self):
        ...
        tk.Button(self.root, text="Connect",
                  command=self.connect).grid(row=5, column=1)
        ...
        tk.Label(self.root, text="Enter a value for x :").grid(
            row=10, column=0)
        self.entry_cx = tk.Entry(self.root, validate="key", validatecommand=(
            self.root.register(self.validate_number), "%P"))  # To be sure that the entry is a number
        ...
        tk.Button(self.root, text="Save", command=lambda: self.save_value(
            self.entry_cx)).grid(row=10, column=2)  # Saves it

        tk.Label(self.root, text="Enter a value for y :").grid(
            row=10, column=4)
        ...
        tk.Button(self.root, text="Save", command=lambda: self.save_value(
            self.entry_cy)).grid(row=10, column=6)  # Saves it
        ...
        tk.Button(self.root, text="Save", command=lambda: self.save_value(
            self.entry_duration)).grid(row=9, column=2)  # Saves it
        ...
        tk.Button(self.root, text="Save", command=lambda: self.save_value(
            self.increment)).grid(row=9, column=5)  # Saves it
        ...
        # Label to display current X position
        self.x_pos = tk.Label(self.root, text="0")
        self.x_pos.grid(row=13, column=1)
        # Label to display current Y position
...
###########################################################################################################################
    ...
    def save_value(self, entry_widget):
        ...
        # To check if the value is a positive floating point number
...
# To perform the step in the snake movement
    def perform_step(self, direction):
        ...
        if self.paused:
            print("Movement paused at step {}, coordinate ({}, {})".format(
                direction, self.cx, self.cy))
            self.pending_action = lambda: self.perform_step(direction)
            return

        # The maximum max_steps that we could reach with the same measuring system would be 35mm
        max_steps = self.grid_size * self.steps_per_mm

        x_corr = self.cx - float(self.entry_cx.get()) * self.steps_per_mm
        y_corr = self.cy - float(self.entry_cy.get()) * self.steps_per_mm

        # The last cell is already scanned thanks to start_detector and the after() methods in the loops below
...
# To handle detector logging
    def log_data(self, x, y, duration):
        ...
        filename = 'logs/scan_x{}_y{}_{}.bin'.format(
            x-float(self.entry_cx.get()), y-float(self.entry_cy.get()), timestamp)
...
# To start the detector
    def start_detector(self, direction):
        ...
        if time.time() - self.time_since_beginning > -1/self.decay_constant * log(0.5) and self.increment_time == True:
            print(
                "Auto-pause at direction {} (x={}, y={})".format(direction, self.cx, self.cy))
            self.paused = True
