...
try:
    ...
    try:
        ...
        raise ImportError("Please install zaber-motion library: pip install zaber-motion")
    ...
    def connect_stages(self):
        ...
        if LIBRARY_TYPE == "zaber":
            ...
            if len(device_list) < 2:
                raise Exception("Expected 2 X-LSM100 stages (X and Y), found {}".format(len(device_list)))
            ...
            # Set movement speed
            self.x_axis.settings.set("maxspeed", self.speed, Units.VELOCITY_MILLIMETRES_PER_SECOND)
            self.y_axis.settings.set("maxspeed", self.speed, Units.VELOCITY_MILLIMETRES_PER_SECOND)
    ...
    def cleanup_stages(self):
        ...
        try:
            if LIBRARY_TYPE == "zaber" and self.connection:
                ...
                if self.x_axis:
                    self.x_axis.move_absolute(0, Units.LENGTH_MILLIMETRES, wait_until_idle=False)
                if self.y_axis:
                    self.y_axis.move_absolute(0, Units.LENGTH_MILLIMETRES, wait_until_idle=False)
...
###########################################################################################################################
    ...
    def move_to_position(self):
        ...
        if LIBRARY_TYPE == "zaber":
            ...
            self.x_axis.move_absolute(self.cx, Units.LENGTH_MILLIMETRES, wait_until_idle=False)
            self.y_axis.move_absolute(self.cy, Units.LENGTH_MILLIMETRES, wait_until_idle=False)
